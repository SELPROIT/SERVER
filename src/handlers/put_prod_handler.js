...
const put_prod_handler= async (req, res) => {
  const { id } = req.params
  const { image, description, datasheet, rating, stock, price } = req.body;
  try {
    const product = await put_prod_controller(id, image, description, datasheet, rating, stock, price)
